...
export function Receipts({ api, setTag, setViewer }: { api: Api; setTag: (t: TagState) => void; setViewer: (v: ViewerState) => void }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ border: '2px dashed var(--color-divider)', padding: 16, display: 'flex', alignItems: 'center', gap: 12, marginBottom: 22, flexWrap: 'wrap' }}>
              ...
              <div key={r.id} style={{ border: '1px solid ' + (r.cat ? 'var(--color-divider)' : 'var(--color-accent)'), background: r.cat ? 'var(--color-bg)' : 'var(--color-accent-100)' }}>
                ...
                <div style={{ padding: '10px 12px' }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, marginTop: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                    ...
                    {r.cat && (
                      <button className="navlink linkbtn" style={{ fontSize: 11, marginLeft: 'auto' }} onClick={() => { delFile(r.id); mut((dd) => { dd.receipts = dd.receipts.filter((q) => q.id !== r.id); }); }}>Delete</button>
                    )}
                  </div>
                </div>
              </div>
